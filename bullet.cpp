...
Bullet::Bullet(vec3f shipPosition, vec3f shipForward, vec3f shipLeft, vec3f shipRight, animation anim, int gun) {
    if (gun == 1) {
        position.x = shipPosition.x+(shipForward.x*3);
        position.y = shipPosition.y+(shipForward.y*3);
        position.z = shipPosition.z+(shipForward.z*3);
    } else if (gun == 2) { //Left
        position.x = shipPosition.x+(shipForward.x*3)+(shipLeft.x*1.4);
        position.y = shipPosition.y+(shipForward.y*3)+(shipLeft.y*1.4);
        position.z = shipPosition.z+(shipForward.z*3)+(shipLeft.z*1.4);
    } else if (gun == 3) { //Right
        position.x = shipPosition.x+(shipForward.x*3)+(shipRight.x*1.4);
        position.y = shipPosition.y+(shipForward.y*3)+(shipRight.y*1.4);
        position.z = shipPosition.z+(shipForward.z*3)+(shipRight.z*1.4);
    }
    forward = shipForward;
    speed = 100.0;
    scale.x = 1.0;
...
vec3f Bullet::getPosition() {
    return position;
}

bool Bullet::maxDistanceTravelled() {
    if (position.x > MAX_BULLET_DISTANCE || position.y > MAX_BULLET_DISTANCE || position.z > MAX_BULLET_DISTANCE || position.x < -MAX_BULLET_DISTANCE || position.y < -MAX_BULLET_DISTANCE || position.z < -MAX_BULLET_DISTANCE) {
        return true;
    }
    return false;
}

void Bullet::billboardCheatSphericalBegin() {
